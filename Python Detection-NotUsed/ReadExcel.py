...
video_path = ["D:\Video\Clear_video.mp4"]
...
for s in wb.sheets():
  ...
  for row in range(2,s.nrows):
    if(s.cell(row,1).value!=''):
      valuesX.append(s.cell(row,1).value)
  for row in range(2,s.nrows):
    if(s.cell(row,2).value!=''):
      valuesY.append(s.cell(row,2).value)
  for row in range(2,s.nrows):
    if(s.cell(row,3).value!=''):
      valuesZ.append(s.cell(row,3).value)

arr1 = np.array(valuesX)
arr2 = np.array(valuesY)
arr3 = np.array(valuesZ)
item = 0
while item < len(valuesX) :
  subvalue=[arr1[item],arr2[item],arr3[item]]
...
##                cv2.line(image,(0,511),(width,511),(255,0,0),5)
                ...
                for e in size_values:
                        ...
                        dataY.append(y)
                        dataZ.append(z)
##                        vector = [sub(float(m),float(n)) for m,n in zip(values,delta)]        
